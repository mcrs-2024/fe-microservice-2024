...
const Accident = () => {
  ...
  const [modalType, setModalType] = useState<AccidentModalType>('add');
  const [selectedRecord, setSelectedRecord] = useState<TAccident | null>();
  ...
  const onChangeModalType = (type: AccidentModalType) => {
    setModalType(type);
  };
  ...
  const inputSearch: InputProps[] = [
    {
      ...
      className: 'w-100',
      allowClear: true,
      value: filter.casualtyName,
      onChange: (e: ChangeEvent<HTMLInputElement>) =>
        onChangeFilter('casualtyName', e.target.value),
    },
  ];

  const columns: ColumnsType<TAccident> = [
    {
    ...
    {
      title: t('Accident'),
      dataIndex: 'casualtyCode',
    },
    {
      title: t('name_accident'),
      dataIndex: 'casualtyName',
    },
    {
      title: t('Action'),
      dataIndex: 'action',
