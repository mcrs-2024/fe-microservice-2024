...
import jobTitleApi from 'src/helpers/api/category/jobTitle';
import { yupObject } from 'src/utils/validate';
import * as Yup from 'yup';

interface Props {
  isShow: boolean;
  modalType: JobTitleModalType;
  selectedRecord: TJobTitle | null;
  onHide: () => void;
  onSuccess: () => void;
}
type FormValues =
...
const JobTitleModal = ({
  ...
  const formControl = useFormik<FormValues>({
    ...
    onSubmit: async (data: FormValues) => {
      try {
        if (modalType === 'add') {
          const res = await jobTitleApi.createJobTitle(data);
          if (res.data.code) {
            onSuccess();
            onHide();
        ...
        if (modalType === 'edit') {
          const res = await jobTitleApi.updateJobTitle(data);
          if (res.data.code) {
            onSuccess();
            onHide();
            message.success(res.data.message);
