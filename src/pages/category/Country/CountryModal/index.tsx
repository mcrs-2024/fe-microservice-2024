...
import countryApi from 'src/helpers/api/category/country';
import { yupObject } from 'src/utils/validate';
import * as Yup from 'yup';

interface Props {
  isShow: boolean;
  modalType: CountryModalType;
  selectedRecord: TCountry | null;
  onHide: () => void;
  onSuccess: () => void;
}
type FormValues =
  ...
  const formControl = useFormik<FormValues>({
    ...
    onSubmit: async (data: FormValues) => {
      try {
        ...
        if (modalType === 'edit') {
          const res = await countryApi.updateCountry(data);
          if (res.data.code) {
            onSuccess();
            onHide();
            message.success(res.data.message);
